(function($) {
	$.fn.filterContent = function(filterField, filterContainer, filterElement) {
		var $containers = $(this);

		var showFilteredElements = function(el,text) {
			if(el.text().toLowerCase().indexOf(text) != -1){
				el.show();
				//el.find('*').show();
			}
		}
			...
			$filterField.keyup( function() {
				...
				if (typeof(element) == "undefined" || element === '') {
					$filterContent.children().each(function(){
						var $me = $(this);
						showFilteredElements($me,srchform.toLowerCase());
					});
				} else {
